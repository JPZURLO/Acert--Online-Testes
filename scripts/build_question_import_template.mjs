...
const previewPath = process.env.PREVIEW_PATH || path.join(projectRoot, "outputs", "question-import-template-preview.png");
...
const headers = [
  ...
  "Alternativa H", "Alternativa I", "Alternativa J", "Imagem", "Texto alternativo", "Resposta correta",
];

questions.showGridLines = false;
questions.getRange("A1:Q1").merge();
questions.getRange("A1").values = [["MODELO DE IMPORTAÇÃO DE QUESTÕES"]];
questions.getRange("A1:Q1").format = {
  fill: "#0F6F73",
  font: { bold: true, color: "#FFFFFF", size: 16 },
  horizontalAlignment: "center",
  verticalAlignment: "center",
};
questions.getRange("A1:Q1").format.rowHeight = 34;
questions.getRange("A2:Q2").merge();
...
questions.getRange("A2:Q2").format = {
  fill: "#E8F3F3",
  font: { color: "#46515B", italic: true },
  horizontalAlignment: "left",
  verticalAlignment: "center",
};
questions.getRange("A2:Q2").format.rowHeight = 27;
questions.getRange("A4:Q4").values = [headers];
questions.getRange("A4:Q4").format = {
  fill: "#46515B",
...
questions.getRange("A4:Q4").format.rowHeight = 32;
questions.getRange("A5:Q204").format = {
  font: { color: "#26323C" },
...
questions.getRange("O5:P204").format.fill = "#F4FBFF";
questions.getRange("Q5:Q204").format.fill = "#FFF8E8";
...
questions.getRange("O:O").format.columnWidth = 42;
questions.getRange("P:P").format.columnWidth = 32;
questions.getRange("Q:Q").format.columnWidth = 26;
questions.freezePanes.freezeRows(4);
questions.getRange("A4:Q204").format.autofitRows();
...
instructions.getRange("A3:F11").values = [
  ...
  ["Imagem", "Não", "Link ou data:image", "Link ou data:image", "Link ou data:image", "Use URL https:// ou data:image/png|jpeg|webp;base64. Também é possível anexar manualmente no editor."],
  ["Texto alternativo", "Não", "Descrição da imagem", "Descrição da imagem", "Descrição da imagem", "Ajuda acessibilidade e revisão da questão."],
...
instructions.getRange("A4:F11").format = {
  wrapText: true, verticalAlignment: "top",
  borders: { preset: "inside", style: "thin", color: "#DDE4E8" },
};
instructions.getRange("A4:A11").format.font = { bold: true, color: "#0F6F73" };
instructions.getRange("A13:F13").merge();
instructions.getRange("A13").values = [["IMPORTANTE: não renomeie a aba Questões nem altere os títulos das colunas. Salve o arquivo no formato .xlsx antes de importar."]];
...
instructions.getRange("A3:F11").format.autofitRows();
instructions.freezePanes.freezeRows(3);

examples.showGridLines = false;
examples.getRange("A1:Q1").merge();
examples.getRange("A1").values = [["EXEMPLOS — NÃO SÃO IMPORTADOS"]];
examples.getRange("A1:Q1").format = {
  fill: "#0F6F73", font: { bold: true, color: "#FFFFFF", size: 16 },
  horizontalAlignment: "center", verticalAlignment: "center",
};
examples.getRange("A1:Q1").format.rowHeight = 34;
examples.getRange("A3:Q3").values = [headers];
examples.getRange("A4:Q6").values = [
  ["Múltipla escolha", "Qual alternativa representa uma comunicação clara?", 20, "Sim", "Mensagem objetiva", "Uso excessivo de termos técnicos", "Ausência de retorno", "", "", "", "", "", "", "", "https://exemplo.com/grafico.png", "Gráfico de apoio da questão", "A"],
...
examples.getRange("A3:Q3").format = {
  fill: "#46515B", font: { bold: true, color: "#FFFFFF" },
  horizontalAlignment: "center", verticalAlignment: "center", wrapText: true,
};
examples.getRange("A4:Q6").format = {
  wrapText: true, verticalAlignment: "top",
  borders: { preset: "inside", style: "thin", color: "#DDE4E8" },
};
...
examples.getRange("O:O").format.columnWidth = 42;
examples.getRange("P:P").format.columnWidth = 32;
examples.getRange("Q:Q").format.columnWidth = 26;
examples.getRange("A3:Q6").format.autofitRows();
examples.freezePanes.freezeRows(3);

const inspection = await workbook.inspect({
  kind: "table",
  range: "Questões!A1:Q8",
...
const preview = await workbook.render({ sheetName: "Questões", range: "A1:Q12", scale: 1, format: "png" });
