function abrirModalCriarQuestao(elemento) {
  // 1. Exibir o modal
  const modal = document.getElementById("modal-criar-questao");
...
function fecharModalCriarQuestao() {
  document.getElementById("modal-criar-questao").style.display = "none";
}
...
function mostrarOpcoesResposta(select) {
    ...
    const opcoesResposta = select.parentNode.querySelector("#opcoes-resposta-criar"); // Altera o seletor
    opcoesResposta.innerHTML = "";

    const respostaDissertativa = document.getElementById("resposta-dissertativa-criar");
    respostaDissertativa.style.display = "none"; // Esconde o campo de texto
    ...
    } else if (tipoPergunta === "dissertativa") {
        respostaDissertativa.style.display = "block"; // Exibe o campo de texto
    }
}

function adicionarPergunta() {
    const perguntasContainer = document.getElementById("perguntas-container-criar"); // Altera o seletor
    ...
    pergunta.innerHTML = `
        ...
        <div id="opcoes-resposta-criar"> </div> <div> </div>
        <div id="resposta-dissertativa-criar" style="display: none;">
            <label for="resposta">Resposta:</label><br>
            <textarea id="resposta-dissertativa-texto-criar"></textarea>
        </div>
    `;
...
function excluirPergunta() {
  const perguntasContainer = document.getElementById("perguntas-container-criar");
